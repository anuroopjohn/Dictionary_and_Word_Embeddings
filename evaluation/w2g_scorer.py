...
def sent_bleu(pred, target, smoothing_function=SmoothingFunction().method4):
    return sentence_bleu([pred], target, smoothing_function=smoothing_function)

def sense_bleu(references, predictions):
    
    sbleus = []
    for sub, ref in zip(predictions, references):
        sub = word_tokenize(sub)
        ref = word_tokenize(ref)
        if len(sub)==0 or len(ref)<=1:
            sbleus.append(0.0)
        else:
            sbleus.append(sent_bleu(sub,ref))

    return sum(sbleus)/len(sbleus),sbleus, list(zip(predictions,references,sbleus))
...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, stop_words=[], n_gram=1, remove_subwords = True, batch_size=256):
    preds = []
    for batch_start in tqdm.tqdm(range(0, len(refs), batch_size)):
